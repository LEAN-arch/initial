...
try:
    st.markdown(
        '<div class="header">Ethical Lean Audit</div>' if st.session_state.lang == "English" else
    ...
    if st.button("Generate Report" if st.session_state.lang == "English" else "Generar Informe", key="generate_report"):
        ...
        if incomplete_categories:
            ...
            try:
                pdf = FPDF()
                pdf.add_page()
                pdf.set_font("Helvetica", "B", 16)  # Use Helvetica as a standard font
                pdf.cell(
                    0, 10,
                ...
                pdf.cell(0, 10, "Summary" if st.session_state.lang == "English" else "Resumen", 0, 1)
                pdf.set_font("Helvetica", "", 12)
                for _, row in df.iterrows():
                    pdf.cell(0, 8, f"{row['Category']}: {row['Score']}/{len(questions[row['Category']][st.session_state.lang])*5} ({row['Percent']:.1f}%)", 0, 1)
                
                pdf.ln(10)
                pdf.set_font("Helvetica", "B", 14)
                pdf.cell(0, 10, "Detailed Results" if st.session_state.lang == "English" else "Resultados Detallados", 0, 1)
                pdf.set_font("Helvetica", "", 12)
                ...
                href = (
                    ...
                    f'{"Download PDF Report" if st.session_state.lang == "English" else "Descargar Informe PDF"}</a>'
                )
