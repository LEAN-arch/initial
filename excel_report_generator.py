...
def generate_excel_report(
    ...
    """
    Generate an Excel report with all content consolidated into a single worksheet using direct xlsxwriter methods.
    ...
    worksheet = workbook.add_worksheet("Audit Report" if language == "English" else "Informe de Auditoría")
    logger.debug("Single worksheet created: %s", worksheet.name)
    ...
    worksheet.write(current_row, 0, translations[language]["metric"], header_format)
    worksheet.write(current_row, 1, translations[language]["value"], header_format)
    current_row += 1
    worksheet.write(current_row, 0, translations[language]["overall_score"], cell_format)
    worksheet.write(current_row, 1, f"{overall_score:.1f}%", cell_format)
    current_row += 1
    worksheet.write(current_row, 0, translations[language]["grade"], cell_format)
    worksheet.write(current_row, 1, grade, cell_format)
    current_row += 2
    ...
    for col_num, col_name in enumerate(results_data.columns):
        worksheet.write(current_row, col_num, col_name, header_format)
    current_row += 1
    for row_num, row_data in results_data.iterrows():
    ...
    if findings_data:
        ...
        for col_num, col_name in enumerate(findings_df.columns):
            worksheet.write(current_row, col_num, col_name, header_format)
        current_row += 1
        for row_num, row_data in findings_df.iterrows():
            for col_num, cell_data in enumerate(row_data):
                worksheet.write(current_row, col_num, cell_data, cell_format)
            current_row += 1
    else:
    ...
    if insights_data:
        ...
        for col_num, col_name in enumerate(insights_df.columns):
            worksheet.write(current_row, col_num, col_name, header_format)
        current_row += 1
        for row_num, row_data in insights_df.iterrows():
            for col_num, cell_data in enumerate(row_data):
                worksheet.write(current_row, col_num, cell_data, cell_format)
            current_row += 1
    else:
    ...
    for col_num, col_name in enumerate(chart_data.columns):
        worksheet.write(current_row, col_num, col_name, header_format)
    current_row += 1
    for row_num, row_data in chart_data.iterrows():
        for col_num, cell_data in enumerate(row_data):
            worksheet.write(current_row, col_num, cell_data, cell_format if col_num == 0 else percent_format)
        current_row += 1
    current_row += 1
    ...
    for col_num, col_name in enumerate(contact_df.columns):
        worksheet.write(current_row, col_num, col_name, header_format)
    current_row += 1
    for row_num, row_data in contact_df.iterrows():
        for col_num, cell_data in enumerate(row_data):
            worksheet.write(current_row, col_num, cell_data, cell_format)
        current_row += 1
    current_row += 1
    ...
    logger.debug("Verified single worksheet: %s", worksheet.name)
